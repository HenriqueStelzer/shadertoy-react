...
;// ./src/glFormats.js
var isWebGL2 = function isWebGL2(gl) {
  return !!gl.createVertexArray;
};
var rgbaInternalFormat = function rgbaInternalFormat(gl) {
  var _ref = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {},
    _ref$float = _ref["float"],
    _float = _ref$float === void 0 ? false : _ref$float;
  return isWebGL2(gl) ? _float ? gl.RGBA32F : gl.RGBA8 : gl.RGBA;
};
...
var KeyboardTexture = /*#__PURE__*/_createClass(function KeyboardTexture(_gl) {
  ...
  _defineProperty(this, "uploadData", function (texture) {
    ...
    gl.texImage2D(gl.TEXTURE_2D, 0, rgbaInternalFormat(gl), KEYBOARD_WIDTH, KEYBOARD_HEIGHT, 0, gl.RGBA, gl.UNSIGNED_BYTE, _this.data);
  });
...
/** Shadertoy-style image uploads flip Y by default. Pass `flipY: false` or `0` to disable. */
var toGlFlipY = function toGlFlipY(flipY) {
  return flipY === false || flipY === 0 ? 0 : 1;
};
...
var resolveTextureUrl = function resolveTextureUrl(textureArgs) {
  ...
  var densities = Object.keys(srcSet).map(Number).filter(function (d) {
...
var Texture = /*#__PURE__*/Texture_createClass(function Texture(_gl) {
  ...
  Texture_defineProperty(this, "loadDataTexture", function (textureArgs) {
    ...
    if (format === "rgba32f") {
      ...
      gl.texImage2D(gl.TEXTURE_2D, 0, rgbaInternalFormat(gl, {
        "float": true
      }), width, height, 0, gl.RGBA, gl.FLOAT, data);
    } else {
      gl.texImage2D(gl.TEXTURE_2D, 0, rgbaInternalFormat(gl), width, height, 0, gl.RGBA, gl.UNSIGNED_BYTE, data);
    }
...
var FramebufferPool = /*#__PURE__*/FramebufferPool_createClass(function FramebufferPool(_gl) {
  ...
  FramebufferPool_defineProperty(this, "getOrCreate", function (name, width, height) {
    ...
    gl.texImage2D(gl.TEXTURE_2D, 0, rgbaInternalFormat(gl), width, height, 0, gl.RGBA, gl.UNSIGNED_BYTE, null);
...
var clearColorEqual = function clearColorEqual(a, b) {
  return JSON.stringify(a || [0, 0, 0, 1]) === JSON.stringify(b || [0, 0, 0, 1]);
};
var propsAffectingScene = function propsAffectingScene(prev, next) {
  return prev.style !== next.style || prev.webgl !== next.webgl || prev.fs !== next.fs || prev.vs !== next.vs || prev.devicePixelRatio !== next.devicePixelRatio || prev.lerp !== next.lerp || prev.precision !== next.precision || !definesEqual(prev.defines, next.defines) || !passesEqual(prev.passes, next.passes) || !texturesEqual(prev.textures, next.textures) || !uniformsSchemaEqual(prev.uniforms, next.uniforms) || !clearColorEqual(prev.clearColor, next.clearColor) || !persistentTimeEqual(prev.persistentTime, next.persistentTime);
};
var isTextureChannelUniform = function isTextureChannelUniform(uniform) {
  return uniform.startsWith(UNIFORM_CHANNEL) && uniform !== UNIFORM_CHANNELRESOLUTION && uniform !== UNIFORM_CHANNELTIME;
};
var uniformMatchesSource = function uniformMatchesSource(uniform, uniformSource, channelOffset) {
  if (isTextureChannelUniform(uniform)) {
    var id = parseInt(uniform.slice(UNIFORM_CHANNEL.length), 10);
    if (!Number.isNaN(id)) {
      return uniformSource.includes("".concat(UNIFORM_CHANNEL).concat(id + channelOffset));
    }
  }
  return uniformSource.includes(uniform);
};
var shaderUniformName = function shaderUniformName(uniform, channelOffset) {
  if (isTextureChannelUniform(uniform) && channelOffset > 0) {
    var id = parseInt(uniform.slice(UNIFORM_CHANNEL.length), 10);
    if (!Number.isNaN(id)) {
      return "".concat(UNIFORM_CHANNEL).concat(id + channelOffset);
    }
  }
  return uniform;
};
var GlslCanvas = /*#__PURE__*/function (_Component) {
  function GlslCanvas(props) {
    ...
    src_defineProperty(_this, "bootstrapScene", function () {
      ...
      _this.onResize();
      var hasTextures = (_this.props.textures || []).length > 0;
      if (!hasTextures) {
    ...
    src_defineProperty(_this, "addEventListeners", function () {
      ...
      if (typeof ResizeObserver !== "undefined") {
        ...
        _this.resizeObserver.observe(_this.canvas);
      }
    });
    ...
    src_defineProperty(_this, "toShaderPixelCoords", function (clientX, clientY) {
      var rect = _this.canvas.getBoundingClientRect();
      var scaleX = _this.canvas.width / rect.width;
      var scaleY = _this.canvas.height / rect.height;
      var x = (clientX - rect.left) * scaleX;
      var y = (rect.height - (clientY - rect.top)) * scaleY;
      return [x, y];
    });
    src_defineProperty(_this, "mouseDown", function (e) {
      ...
      var _this$toShaderPixelCo = _this.toShaderPixelCoords(clientX, clientY),
        _this$toShaderPixelCo2 = _slicedToArray(_this$toShaderPixelCo, 2),
        mouseX = _this$toShaderPixelCo2[0],
        mouseY = _this$toShaderPixelCo2[1];
    ...
    src_defineProperty(_this, "mouseMove", function (e) {
      ...
      var clientX = e.clientX || e.changedTouches[0].clientX;
      var clientY = e.clientY || e.changedTouches[0].clientY;
      var _this$toShaderPixelCo3 = _this.toShaderPixelCoords(clientX, clientY),
        _this$toShaderPixelCo4 = _slicedToArray(_this$toShaderPixelCo3, 2),
        mouseX = _this$toShaderPixelCo4[0],
        mouseY = _this$toShaderPixelCo4[1];
      if (lerp !== 1) {
        _this.lastMouseArr[0] = mouseX;
        _this.lastMouseArr[1] = mouseY;
      } else {
    ...
    src_defineProperty(_this, "syncCanvasSize", function () {
      var _this6 = _this,
        gl = _this6.gl;
      ...
      _this.canvasPosition = _this.canvas.getBoundingClientRect();
      var displayWidth = Math.max(1, Math.round(_this.canvasPosition.width * devicePixelRatio));
      var displayHeight = Math.max(1, Math.round(_this.canvasPosition.height * devicePixelRatio));
    ...
    src_defineProperty(_this, "drawMultiPass", function (timestamp) {
      ...
      passes.forEach(function (pass, index) {
        ...
        var inputCount = (pass.inputs || []).length;
        _this.texturesArr.forEach(function (texture, id) {
          if (!texture.isLoaded) return;
          var textureUniformKey = "".concat(UNIFORM_CHANNEL).concat(id);
          if (!_this.uniforms[textureUniformKey] || !_this.uniforms[textureUniformKey].isNeeded) {
            return;
          }
          var channelIdx = inputCount + id;
          ...
          var loc = gl.getUniformLocation(program, "".concat(UNIFORM_CHANNEL).concat(channelIdx));
    ...
    src_defineProperty(_this, "compileShaders", function () {
      ...
      if (passes && passes.length > 0) {
        ...
        _this.passPrograms = passes.map(function (pass) {
          var inputCount = (pass.inputs || []).length;
          var shaders = _this.preProcessShaders(pass.fs, vs || defaultVs, pass.fs, inputCount, inputCount);
    ...
    src_defineProperty(_this, "processTextures", function () {
      ...
      Promise.all(texturePromisesArr).then(function () {
        ...
        console.error(e);
        if (!isUpdate) _this.compileShaders();
        if (onDoneLoadingTextures) onDoneLoadingTextures();
      });
    });
    src_defineProperty(_this, "preProcessShaders", function (fs, vs, uniformSource) {
      var channelOffset = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : 0;
      var inputChannelCount = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : 0;
      ...
      for (var i = 0; i < inputChannelCount; i++) {
        var channelName = "".concat(UNIFORM_CHANNEL).concat(i);
        if (uniformSource.includes(channelName)) {
          fsString = insertStringAtIndex(fsString, "uniform sampler2D ".concat(channelName, "; \n"), uniformInsertIndex);
        }
      }
      Object.keys(_this.uniforms).forEach(function (uniform) {
        if (!uniformMatchesSource(uniform, uniformSource, channelOffset)) return;
        var declaredName = shaderUniformName(uniform, channelOffset);
        fsString = insertStringAtIndex(fsString, "uniform ".concat(_this.uniforms[uniform].type, " ").concat(declaredName).concat(_this.uniforms[uniform].arraySize || "", "; \n"), uniformInsertIndex);
    ...
    src_defineProperty(_this, "resizeObserver", null);
  ...
  return src_createClass(GlslCanvas, [{
    ...
    value: function componentDidUpdate(prevProps) {
      ...
      var precisionChanged = prevProps.precision !== this.props.precision;
      ...
      if (fsChanged || vsChanged || definesChanged || passesChanged || uniformsSchemaChanged || precisionChanged) {
